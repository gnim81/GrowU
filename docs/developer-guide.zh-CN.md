...
本文档面向公开发布后参与 GrowU 开发的维护者、贡献者和 AI 工具。它描述了当前架构、关键业务规则以及定义产品持久行为的文件。

## 项目概览

GrowU 是一个单一仓库的 Next.js 应用，用于追踪孩子的正向强化、积分变化和奖励兑换。它使用服务端渲染的 App Router 前端、Server Actions 处理写操作、Prisma 持久化数据、PostgreSQL 作为系统记录存储。
...
GrowU 现在使用基于数据库的 `UserAccount` 记录，而非环境变量定义的账户列表。
